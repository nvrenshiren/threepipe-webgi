...
import { Color, HalfFloatType, LinearFilter, Material, NoColorSpace, RGBAFormat, Texture, UnsignedByteType, WebGLRenderTarget } from "three"
...
export class ExtendedRenderPass extends RenderPass implements IPipelinePass<"render"> {
  ...
  private _releaseTransparentTarget() {
    if (this._transparentTarget) this.renderManager.releaseTempTarget(this._transparentTarget)
  ...
  private _releaseOpaqueTarget() {
    if (this._opaqueTarget) this.renderManager.releaseTempTarget(this._opaqueTarget)
  ...
  render(
    ...
    writeBuffer?: WebGLRenderTarget<Texture | Texture[]> | null,
    readBuffer?: WebGLRenderTarget<Texture | Texture[]>,
    deltaTime?: number,
    maskActive?: boolean
  ) {
    ...
    if (!this.renderManager.rgbm) {
      ...
      if (renderer.info && !renderer.info.autoReset) throw "renderer.info.autoReset must be true" // it is required to check if any object is rendered in the scene, also frame count is maintained separately in the render manager, use that.
      ...
      if (renderToScreen) {
        ...
        source &&
          this.renderManager.blit(undefined, {
            source,
    ...
    if (!needsSwap && lastReadBuffer !== readBuffer && readBuffer) {
      ...
      source &&
        this.renderManager.blit(lastReadBuffer, {
          source: source,
          clear: true
        })
    ...
    if (!this.preserveTransparentTarget) this._releaseTransparentTarget()
    if (!this.preserveOpaqueTarget) this._releaseOpaqueTarget()
