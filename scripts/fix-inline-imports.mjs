...
async function processFile(jsFile) {
  ...
  for (const [ext, replacement] of Object.entries(importMap)) {
    const regex = new RegExp(`(import\\s+[^'"]+['"])(\\.\\/[^'"]+)\\.${ext}(['"])`, "g")
...
async function main() {
  ...
  const jsFiles = (await fs.readdir("./" + distDir, { recursive: true })).filter(file => file.endsWith(".js")).map(file => path.join("./" + distDir, file))
