...
Object.entries(imports).forEach(([k, v]) =>
  k === "three" || k === "threepipe" || k.startsWith("@threepipe/")
    ? (imports[k] = "https://unpkg.com/" + (k === "three" ? "threepipe" : k) + "/dist/" + v.split("/dist/").pop())
    : ""
) // required for codepen to work. this is done because plugins refer to threepipe as esm
